...
export const genMock = ({mockType, genArray}: IMockConfig): any => {
    ...
    const genStateUser = (): UserState => {
        return {
            ...
            emails: [{_id: faker.random.uuid(), value: faker.internet.email()}],

            messages: [faker.random.uuid()],

            photos: [{_id: faker.random.uuid(), value: faker.image.imageUrl()}],

            isAuthorized: true
        }
    };

    const genNetUser = (): Omit<UserState, 'isAuthorized'> => {
        return {
            displayName: faker.internet.userName(),
